...
3. Reserve `ERROR`, `WARN`, and `INFO` for information that is operationally significant.
4. Use `DEBUG`, `FINE`, `FINER`, and `FINEST` for high-volume diagnostic detail. Combined with `LoggerSettings__c.LoggingLevel__c`, these can be left in code without adding runtime overhead in production and switched on only when deeper diagnostics are needed.
5. Tune `LoggerSettings__c.LoggingLevel__c` by environment.
6. Use `Logger.setScenario()` for business-process grouping.
7. Associate records with `.setRecord(...)` instead of embedding IDs in message strings.
8. Define a controlled tag taxonomy (for example: `domain:*`, `feature:*`, `incident:*`).
9. Configure scheduled purging and retention windows (`DefaultLogPurgeAction__c`, `DefaultNumberOfDaysToRetainLogs__c`).
