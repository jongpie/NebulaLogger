...
//    nebula-logger/core/main/logger-engine. Custom objects, CMDT, and
//    triggers are excluded entirely - custom objects/CMDT are generated by
//    scripts/dev/metadata-reference-generator/ which parses the raw XML
//    directly. LogBatchPurger + LogBatchPurgeScheduler are global but live
//    under log-management; those are excluded intentionally.
//
...
// docs/src/content/docs/metadata-reference/. Frontmatter is injected via
...
export default defineMarkdownConfig({
  ...
  targetDir: 'docs/src/content/docs/metadata-reference',
  scope: ['global'],
  customObjectVisibility: ['public'],
  sortAlphabetically: true,
  linkingStrategy: 'relative',
  referenceGuideTitle: 'Metadata Reference',
  exclude: [
  ...
  transformDocs: docs => docs.filter(doc => doc.type !== 'customobject' && doc.type !== 'trigger'),
  transformReferenceGuide: guide => {
    ...
    return {
      frontmatter: {
        ...
        description: 'Auto-generated metadata reference for Nebula Logger Apex classes, custom objects, triggers, and LWCs.'
      },
      content: intro
    };
  },
  transformDocPage: doc => {
    const title = doc.source?.name ?? doc.frontmatter?.title ?? 'Metadata Reference';
