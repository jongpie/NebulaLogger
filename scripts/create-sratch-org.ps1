...
$devHubs = $defaultDevHub, 'DevHub2', 'DevHub3'
Write-Output "Pool of possible dev hubs: $devHubs"

foreach($devHub in $devHubs) {
    Write-Output "looking at dev hub: $devHub"
    $scratchOrgAllotment = ((sfdx force:limits:api:display --json --targetusername $defaultDevHub | ConvertFrom-Json).result | Where-Object -Property name -eq "DailyScratchOrgs").remaining
