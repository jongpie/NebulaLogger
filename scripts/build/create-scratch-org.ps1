...
foreach($devHub in $devHubs) {
    ...
    $scratchOrgAllotment = ((npx sf limits api display --json --target-org $devHub | ConvertFrom-Json).result | Where-Object -Property name -eq "DailyScratchOrgs").remaining
    ...
    if($scratchOrgAllotment -gt 0) {
        ...
        # Create Scratch Org
        try {
            $scratchOrgCreateMessage = npx sf org create scratch --set-default --target-dev-hub $devHub --duration-days $durationdays --definition-file $definitionFile --wait 10
